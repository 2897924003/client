<template>
  ...
  <q-page class="q-pa-md">
    <q-toolbar>
      ...
      <q-input v-model="search" placeholder="暂未实现..." outlined @keyup.enter="applyFilter">
      ...
      <q-fab color="amber" text-color="black" icon="keyboard_arrow_left" direction="right" @click="drawer = !drawer">
      ...
      <q-drawer
        v-model="drawer"
        side="right"
        :width="$q.screen.width > 600 ? 1500 : 300"
        class="flex row"
        show-if-above
        >
        ...
        <q-splitter v-model="splitterModel"
          ...
          <template v-slot:after>
            ...
            >
              <template v-slot:append>
                <q-icon v-if="filter !== ''" name="search" class="cursor-pointer" @click="doFilter" />
              </template>
            </q-input>
        ...
        <div class="fit">
          <q-list>
            <q-card v-for="image in images" :key="image.entity">
              <q-card-section>
                <q-img :src="image.path" alt="Drawing Image" class="cursor-pointer" @click="showImage(image)" />
              </q-card-section>
              <q-card-section>
                暂时没有描述
              </q-card-section>
    ...
    <q-dialog v-model="dialog">
      <q-card class="window-width">
        ...
        <q-card-actions align="center">
          ...
          <q-btn flat label="下载" color="primary" v-close-popup @click="download(dialogImage)" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
const fetchProject = async (newVal) => {
  ...
  try {
    const response = await axios.get(`/api/file/projects/${newVal}`);
...
const fetchProjectImg = async (project)=>{
  ...
  try {
    ...
    const response = await axios.get(`/api/file/ids?project=${project}&category=${selectedCategory.value}`);
...
const fetchImages = async () => {
  ...
  try {
    const response = await axios.get(`/api/file/ids?category=${selectedCategory.value}`);
    images.value = response.data.map(entity => ({
      entity,
      path: `https://test.opensun.asia/api/file/image/${entity.id}`,
    }));
